...
async function ensureChat(productId, product, buyerId) {
  ...
  if (!snap.exists()) {
    ...
    await setDoc(refDoc, {
      ...
      sellerWhatsapp: product.sellerWhatsapp || null, buyerWhatsapp: null, buyerEmail: null, buyerFullName: null,
      participants: [buyerId, product.sellerId], lastMessage: "Conversation démarrée",
      lastMessageAt: serverTimestamp(), lastSenderId: buyerId, unreadBy: product.sellerId === buyerId ? [] : [product.sellerId]
    });
  }
  return chatId;
}
...
async function postChatMessage(productId, product, type, text, amount) {
  ...
  await addDoc(collection(db, "chats", chatId, "messages"), {
    senderId: currentUser.uid, senderName: currentUser.displayName || currentUser.email,
    text, type: type || "text", amount: amount || null, readBy: [currentUser.uid], createdAt: serverTimestamp()
  });
  const otherId = currentUser.uid === product.sellerId ? buyerId : product.sellerId;
  await updateDoc(doc(db, "chats", chatId), { lastMessage: text, lastMessageAt: serverTimestamp(), lastSenderId: currentUser.uid, unreadBy: [otherId] });
}

async function markChatRead(chatId) {
  try {
    const q = query(collection(db, "chats", chatId, "messages"), orderBy("createdAt", "desc"), limit(200));
    const snap = await new Promise((resolve, reject) => { const u = onSnapshot(q, x => { u(); resolve(x); }, reject); });
    const writes = snap.docs.filter(d => !(d.data().readBy || []).includes(currentUser.uid));
    await Promise.all(writes.map(d => updateDoc(d.ref, { readBy: arrayUnion(currentUser.uid) })));
    await updateDoc(doc(db, "chats", chatId), { unreadBy: [] });
  } catch (e) { console.warn("Lecture chat non marquée", e); }
}
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        if (dataUrl.length > 700000) {
          reject(new Error("Image encore trop lourde après compression"));
          return;
        }
        resolve(dataUrl);
      };
...
async function uploadChatImage(file) {
  ...
  if (!file.type.startsWith("image/")) { showToast("Choisis une image"); return; }
  if (file.size > 10 * 1024 * 1024) { showToast("Image trop lourde (10 Mo max)"); return; }
  ...
  try {
    ...
    console.error("Erreur photo chat :", e);
...
async function listenSellerPresence(sellerId) {
  if (unsubOtherPresence) unsubOtherPresence();
  if (!sellerId) return;
  unsubOtherPresence = onSnapshot(doc(db, "users", sellerId), snap => {
    const d = snap.data() || {};
    const el = $("detail-seller-sub");
    if (el) el.innerHTML = `Vendeur · ${presenceLabel(d)}`;
  });
}
...
$("btn-open-chat").addEventListener("click", async () => {
  ...
  } catch (err) { console.error(err); showToast("Impossible d'ouvrir la conversation"); }
});
...
async function openChat(chatId) {
  ...
  $("chat-header-avatar").innerHTML = otherPhoto ? `<img src="${otherPhoto}" style="width:100%;height:100%;border-radius:50%;object-fit:cover">` : initials(otherName);
  const waBtn = $("chat-whatsapp-btn");
  const targetWhatsapp = isBuyer ? currentChatMeta.sellerWhatsapp : currentChatMeta.buyerWhatsapp;
  if (targetWhatsapp) { waBtn.style.display = "flex"; waBtn.href = whatsappLink(targetWhatsapp, `Bonjour, à propos de "${currentChatMeta.productTitle}" sur BOULKA.`); }
  else waBtn.style.display = "none";
  const otherId = isBuyer ? currentChatMeta.sellerId : currentChatMeta.buyerId;
  if (unsubOtherPresence) unsubOtherPresence();
  unsubOtherPresence = onSnapshot(doc(db, "users", otherId), snap2 => {
    const e = $("chat-header-status"); if (e) e.innerHTML = presenceLabel(snap2.data() || {});
  });
  showView("view-chat");
...
function messageStatus(m) {
  ...
  return (m.readBy || []).length > 1 ? "✓✓ Vu" : "✓ Envoyé";
}
...
function listenChatMessages(chatId) {
  ...
  unsubChatMessages = onSnapshot(q, async (snap) => {
    ...
    wrap.innerHTML = msgs.map(m => {
      ...
      if (m.type === "image") return `<div class="msg-row ${mine ? "mine" : ""}"><div><div class="msg-bubble image-bubble"><img src="${m.imageUrl}" alt="Image envoyée" class="chat-image"></div><div class="msg-time">${time} ${status}</div></div></div>`;
      if (m.type === "offer") return `<div class="msg-row system"><div class="msg-bubble">💬 ${escapeHtml(m.text)}<div class="msg-time">${time}</div></div></div>`;
      if (m.type === "order") return `<div class="msg-row system"><div class="msg-bubble order-bubble">🛒 ${escapeHtml(m.text).replace(/\n/g, "<br>")}<div class="msg-time">${time}</div></div></div>`;
      return `<div class="msg-row ${mine ? "mine" : ""}"><div><div class="msg-bubble">${escapeHtml(m.text)}</div><div class="msg-time">${time} ${status}</div></div></div>`;
    }).join("");
    wrap.scrollTop = wrap.scrollHeight;
    const unreadIncoming = snap.docs.filter(d => d.data().senderId !== currentUser.uid && !(d.data().readBy || []).includes(currentUser.uid));
    if (unreadIncoming.length) await Promise.all(unreadIncoming.map(d => updateDoc(d.ref, { readBy: arrayUnion(currentUser.uid) }).catch(() => {})));
  }, (err) => { console.error(err); showToast("Impossible de charger la conversation"); });
}

$("chat-form").addEventListener("submit", async (e) => {
  e.preventDefault();
  const input = $("chat-input"); const text = input.value.trim();
  if (!text || !currentChatId) return; input.value = "";
  try {
    await addDoc(collection(db, "chats", currentChatId, "messages"), { senderId: currentUser.uid, senderName: currentUser.displayName || currentUser.email, text, type: "text", readBy: [currentUser.uid], createdAt: serverTimestamp() });
    const otherId = currentUser.uid === currentChatMeta.sellerId ? currentChatMeta.buyerId : currentChatMeta.sellerId;
    await updateDoc(doc(db, "chats", currentChatId), { lastMessage: text, lastMessageAt: serverTimestamp(), lastSenderId: currentUser.uid, unreadBy: [otherId] });
  } catch (err) { console.error(err); showToast("Message non envoyé — réessaie"); }
});

$("chat-image-input").addEventListener("change", e => uploadChatImage(e.target.files?.[0]));

$("btn-back-chat").addEventListener("click", () => {
  if (unsubChatMessages) unsubChatMessages();
  if (unsubOtherPresence) unsubOtherPresence();
  showView("view-app"); switchTab("notif");
});
...
function listenThreads() {
  ...
  unsubNotif = onSnapshot(q, (snap) => {
    ...
    wrap.innerHTML = threads.map(t => {
      ...
      return `<div class="thread-card" data-chat="${t.id}"><div class="avatar">${otherPhoto ? `<img src="${otherPhoto}" style="width:100%;height:100%;border-radius:50%;object-fit:cover">` : initials(otherName)}</div><div class="thread-info"><div class="thread-product">${escapeHtml(t.productTitle || "")}</div><div class="thread-top"><span class="thread-name">${escapeHtml(otherName || "Utilisateur")}</span><span class="thread-time">${time}</span></div><div class="thread-preview">${escapeHtml(t.lastMessage || "")}</div></div>${(t.unreadBy || []).includes(currentUser.uid) ? '<span class="unread-pill">Nouveau</span>' : ''}</div>`;
    }).join("");
...
$("btn-submit-review").addEventListener("click", async () => {
  ...
  const reviewId = `${currentProductId}__${currentUser.uid}`;
  const reviewRef = doc(db, "reviews", reviewId);
  try {
    const existing = await getDoc(reviewRef);
    const legacyQuery = query(collection(db, "reviews"), where("productId", "==", currentProductId), where("buyerId", "==", currentUser.uid), limit(1));
    const legacySnap = await getDocs(legacyQuery);
    if (existing.exists() || !legacySnap.empty) {
      showToast("Tu as déjà laissé un avis sur ce produit");
      return;
    }
...
$("btn-admin-gate").addEventListener("click", openAdminGate);
...
config js " export const firebaseConfig = {
